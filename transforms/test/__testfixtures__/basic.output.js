...
export default Component.extend({
   ...
   * @return {[bool]}
   */
  hasNextQuestion: computed('questionIndex', 'questions', function hasNextQuestion() {
  ...
  init() {
    ...
    this.set(
      ...
      this.get('canViewSocialQuestions') &&
        this.get('lix').getTreatmentIsEnabled('learning.client.social.quiz-qa-redirect')
    );
  ...
  actions: {
    selectOption({ question, optionIndex }) {
      ...
      return quizStatus
        ...
        .then(status => {
          ...
          const responses = status.getWithDefault('details.responses', emberArray());
          ...
          if (questionSubmissions) {
            submissions = questionSubmissions.getWithDefault('submissions', submissions);
          }
          ...
          this.setProperties({
            isAttempted: true,
            canTryAgain: !isCorrect && !withMaximumAttempts && submissions.length > 0,
          });
        });
    },
    ...
    nextQuestion(reset = false) {
      ...
      this.set('reviewQuestionAnimation', reviewQuestionAnimations.nextQuestion);
    ...
    previousQuestion() {
      ...
      this.set('reviewQuestionAnimation', reviewQuestionAnimations.previousQuestion);
    },
